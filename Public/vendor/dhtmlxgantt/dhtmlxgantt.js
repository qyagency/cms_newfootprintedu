...
gantt._tasks_dnd = {
		...
		if(!drag){
			...
			if (drag.mode && drag.mode != gantt.config.drag_mode.ignore && gantt.config["drag_" + drag.mode]){
				...
				if(login_config.permission==user_permiss.none){
					return false;
				}else if(login_config.permission==user_permiss.limit){
					//限制的修改权限
					if(task.end_date.getTime()<now){
						gantt.message({text:"结束的项目", type:"error"});
						return false
					}
					if(drag.mode==gantt.config.drag_mode.move){//改变任务的开始时间小于当前时间的话，不能移动
						if(task.start_date.getTime()<now){
							gantt.message({text:"任务已经开始，可调整期限", type:"error"});
							return false;
						}else{
							check_obj.is_not_start=true;
							check_obj.start_d=task.start_date.getTime();
						}
					}
					if(drag.mode==gantt.config.drag_mode.resize){//改变任务的天数
						if(drag.left){
							if(task.start_date.getTime()<now){
								gantt.message({text:"已开始时间点不可改变", type:"error"});
								return false;
							}else{
								check_obj.is_not_start=true;
								check_obj.start_d=task.start_date.getTime();
	...
	on_mouse_up : function(e){
		...
		if (drag.mode && drag.id){
			...
			//ev.start_date=new Date().toLocaleDateString();

			if(check_obj.is_not_start){
				if(ev.start_date.getTime()<now){
					ev.start_date=new Date(now);
				}
			}
			check_obj.is_not_start=false;
...
gantt.addLink = function(link) {
	...
	//gantt._lpull.push(link);
	//console.log(gantt._lpull);
...
gantt.init = function(node, from, to){
	this.callEvent("onBeforeGanttReady", []);//产生onBeforeGanttReady事件
