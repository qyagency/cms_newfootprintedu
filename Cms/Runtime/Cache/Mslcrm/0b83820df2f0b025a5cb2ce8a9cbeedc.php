...
<head>
    <meta charset="UTF-8">
    <title>
        匿名客户信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <style type="text/css">
        ...
        .banguser li span:nth-child(1){
            width: 40px;
        }
        .banguser li span:nth-child(2){
            width: 100px;
        }
        .banguser li span:nth-child(3){
            width: 160px;
        }
    </style>
    <section class="panel">
        <div class="panel-body">
            ...
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="AnonymousSurvey_<?php echo ($k); ?>">
                        ...
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/AnonymousSurvey/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <a href="javascript:void(0)" data-id="<?php echo ($vo['id']); ?>"  class="btn btn-xs btn-primary ml-sm mr-sm popshow"><i class="fa fa-pencil"></i> 查看</a>
                            <a href="javascript:void(0)" data-id="<?php echo ($vo['id']); ?>"  class="btn btn-xs btn-primary ml-sm mr-sm banding"><i class="fa fa-pencil"></i> 绑定</a>
                        </td>
...
<!-- Specific -->
    ...
    <script>
        ...
        $.ajax({
            url: "http://msl-crm.cms.incker.com/api/getProject",
            type: 'post',
            success: function (res) {
                console.log(res)
                for (var i=0;i<res.list.length;i++){
                    $('#project').append('<option>'+res.list[i]+'</option>')
                }
            },
            error: function () {
                //that.postLock = true;
        ...
        datatable .on('click','.popshow',function () {
            ...
            $('.banguser').hide()
             $.ajax({
                 url: "http://msl-crm.cms.incker.com/api/getUserSurveyDetailsById",
                 type: 'post',
                 data: {"id":id},
                 success: function (res) {
                     console.log(res);
        ...
        datatable.on('click','.banding' ,function () {
           var pro = $($(this).parent().siblings('td')[2]).text(),
               ul = $('.banguser ul'),
               id=$($(this).parent().siblings('td')[0]).text();
           ...
           console.log(pro)
            $.ajax({
                url: "http://msl-crm.cms.incker.com/api/getProjectUserInfo",
                type: 'post',
                data: {
                    user_project:pro
                },
                success: function (res) {
                    for (var i=0;i<res.userList.length;i++) {
                        ul.append('<li data-id="'+ id +'"><span>'+res.userList[i]['id']+'</span><span>'+res.userList[i]['user_name']+'</span><span>'+res.userList[i]['user_email']+'</span>')
                    }
        ...
        $('.banguser').on('click','li',function () {
            var i=$('.banguser li').index($(this)),
                user_id = $($(this).find('span')[0]).text(),
                id = $(this).data('id');

            var cf=confirm('确定绑定该条数据吗？');
            if(cf){
                $.ajax({
                    url: "http://msl-crm.cms.incker.com/api/bindSurvey",
                    type: 'post',
                    data: {
                        id:id,
                        user_id:user_id
                    ...
                    success: function (res) {
                        if (res.code==200){
                            window.location.href="http://msl-crm.cms.incker.com/AnonymousSurvey/list.html";
                        }
                    },
                    error: function () {
                        //that.postLock = true;
        ...
        function change(val) {
            val.change(function () {
                ...
                console.log(v)
                $.ajax({
                    url: "http://msl-crm.cms.incker.com/api/getAnonymousInfo",
                    type: 'post',
                    data:{
                        user_project : v
                    },
                    success: function (res) {
                        ...
                        for (var i=0;i<data[0].length;i++){
                            ...
                            datatable.append('<tr id="AnonymousSurvey_'+(i+1)+'"><td>'+
                                data[0][i]['id']
                                +'</td><td>'+
                                data[0][i]['user_name']
                                +'</td><td>'+
                                ...
                                +'</td><td><a href="/AnonymousSurvey/edit/id/'+
                                data[0][i]['id']
                                +'.html" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a>' +
                                '<a href="javascript:void(0)" data-id="'+data[0][i]['id']+'"  class="btn btn-xs btn-primary ml-sm mr-sm popshow"><i class="fa fa-pencil"></i> 查看</a>'+
                                    '<a href="javascript:void(0)" data-id="'+data[0][i]['id']+'"  class="btn btn-xs btn-primary ml-sm mr-sm banding"><i class="fa fa-pencil"></i> 绑定</a>'
                                + '</td></tr>')
                        }
        ...
        $('#infoPage').on('click','span',function () {
            ...
            for (var i=0;i<page_arr[z].length;i++){
                ...
                datatable.append('<tr id="AnonymousSurvey_'+(i+1)+'"><td>'+
                    ...
                    page_arr[z][i]['id']
                    +'.html" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a>' +
                    '<a href="javascript:void(0)" data-id="'+page_arr[z][i]['id']+'"  class="btn btn-xs btn-primary ml-sm mr-sm popshow"><i class="fa fa-pencil"></i> 查看</a>'+
                    '<a href="javascript:void(0)" data-id="'+page_arr[z][i]['id']+'"  class="btn btn-xs btn-primary ml-sm mr-sm banding"><i class="fa fa-pencil"></i> 绑定</a>'
                    + '</td></tr>')
            }
