...
<head>
    <meta charset="UTF-8">
    <title>
        预定信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
        ...
        <div class="panel-body">
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="booking_<?php echo ($k); ?>">
                        ...
                        <td><?php echo ($vo["booking_name"]); ?></td>
                        <td><?php echo ($vo["booking_mobile"]); ?></td>
                        <td><?php echo (translateRoom($vo["booking_room_id"])); ?></td>
                        <td><?php echo ($vo["booking_date"]); ?></td>
                        <td><?php echo (translateTime($vo["booking_start_time"])); ?></td>
                        <td><?php echo (translateTime($vo["booking_end_time"])); ?></td>
                        <td><?php echo (translateStatus($vo["booking_status"])); ?></td>
                        <td><?php echo ($vo["booking_remark"]); ?></td>
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/Booking/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="booking_<?php echo ($k); ?>" data-href="<?php echo U('/Booking/delete/id/'.$vo['id']);?>"
