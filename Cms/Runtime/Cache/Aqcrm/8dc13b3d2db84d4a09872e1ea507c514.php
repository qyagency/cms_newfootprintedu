...
<head>
    <meta charset="UTF-8">
    <title>
        预定状态 - 编辑 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <section class="panel">
        <div class="panel-body">
            <div class="form-group">   <label class="col-md-2 control-label">预定状态:<span class="required">*</span></label>   <div class="col-md-8">       <select data-plugin-selectTwo class="form-control" name="<?php echo (CONTROLLER_NAME); ?>Booking_status" required  >               <option value="">请选择</option>           <?php if(is_array($vo["StatusList"])): $i = 0; $__LIST__ = $vo["StatusList"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vos["key"]); ?>" <?php echo ($vos["readonly"]); ?> <?php if(($vos["key"]) == $vo["booking_status"]): ?>selected<?php endif; ?>><?php echo ($vos["label"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>       </select>   </div></div>
        </div>
