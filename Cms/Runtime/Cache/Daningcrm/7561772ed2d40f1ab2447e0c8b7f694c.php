...
<head>
    <meta charset="UTF-8">
    <title>
        博爱家园申请信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
        ...
        <div class="panel-body">
            ...
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="SurveyInfo_<?php echo ($k); ?>">
                        ...
                        <td>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="SurveyInfo_<?php echo ($k); ?>" data-href="<?php echo U('/UserApplyInfo/delete/id/'.$vo['id']);?>"
    ...
    <script>
        ...
        $('#out').on('click',function () {
            ...
            if (confirm("是否导出《"+v+"》")){
                $(window).attr('location','https://daning-crm.cms.incker.com/api/exportExcelForApply/apply_name/' + v);
            }
