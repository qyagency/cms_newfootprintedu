        ...
        <div class="panel-body">
            ...
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="ActivityInfo_<?php echo ($k); ?>">
                        ...
                        <td class="add"><?php echo ($vo["activity_can_use_time"]); ?></td>
                        <td class="add"><?php echo ($vo["activity_address"]); ?></td>
                        <td><img id="news_img" height="50px" width="80px" src="<?php echo ($vo["activity_head_img"]); ?>" class="pimg" alt="活动图片" /></td>
                        <td class="add"><?php echo ($vo["activity_details"]); ?></td>
                        <td><?php echo ($vo["activity_sort"]); ?></td>
                        <td><?php echo (translateArea($vo["is_verify"])); ?></td>
                        <td><?php echo (translateArea($vo["is_confirm"])); ?></td>
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/ActivityInfo/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="ActivityInfo_<?php echo ($k); ?>" data-href="<?php echo U('/ActivityInfo/delete/id/'.$vo['id']);?>"
                                   data-toggle="confirmation" class="btn btn-xs btn-primary"><i
                                        class="fa fa-trash-o"></i> 删除</a><?php endif; ?>
                            <a href="javascript:void(0)" id = "activity_<?php echo ($vo['id']); ?>"  data-id="<?php echo ($vo['id']); ?>" class="activityLink btn btn-xs ml-sm mr-sm ">活动链接</a>
                        </td>
...
<!-- Specific -->
    ...
    <script>
        ...
        $('.activityLink').click(function () {
            ...
            alert("pages/activity/detail?id="+activity_id)
        });
        ...
        $('#projectList,#keyword').change(function () {
            ...
            $.ajax({
                url:'https://daning-crm.cms.incker.com/api/getActivityInfo',
                type:'post',
                data:{
                    'search_value':ik
                },
        ...
        function append(e,data) {
            ...
            if (data.length>0){
                ...
                for (var i=0;i<data[e].length;i++){
                    ...
                    var verify = '',confirm='',
                        id = data[e][i].id,
                        name = data[e][i]['activity_name'],
                        time = data[e][i]['activity_can_use_time'],
                        address = data[e][i]['activity_address'],
                        img = data[e][i]['activity_img'].split(',')[0],
                        details = data[e][i]['activity_details'],
                        sort = data[e][i]['activity_sort'];
                    ...
                    tbody.append('<tr id="ActivityInfo_'+id+ '" role="row" class="odd">\n' +
                        '                        <td class="sorting_1">'+id+'</td>\n' +
                        '                        <td>'+name+'</td>\n' +
                        '                        <td class="add">'+time+'</td>\n' +
                        '                        <td class="add">'+address+'</td>\n' +
                        '                        <td><img id="news_img" height="50px" width="80px" src="'+img+'" class="pimg" alt="活动图片"></td>\n' +
                        '                        <td class="add">'+details+'</td>\n' +
                        '                        <td>'+sort+'</td>\n' +
                        '                        <td>'+verify+'</td>\n' +
                        '                        <td>'+confirm+'</td>\n' +
                        '                        <td>\n' +
                        '                            <a href="/ActivityInfo/edit/id/'+id+'.html" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a>                            <a data-target="ActivityInfo_'+id+'" data-href="/ActivityInfo/delete/id/'+id+'.html" data-toggle="confirmation" class="btn btn-xs btn-primary" data-original-title="" title=""><i class="fa fa-trash-o"></i> 删除</a>                            <a href="javascript:void(0)" id="activity_'+id+'" data-id="'+id+'" class="activityLink btn btn-xs ml-sm mr-sm ">活动链接</a>\n' +
                        '                        </td>\n' +
                        '                    </tr>')
