...
<head>
    <meta charset="UTF-8">
    <title>
        头图信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
    ...
    <section class="panel">
        <div class="panel-body">
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="BannerInfo_<?php echo ($k); ?>">
                        <td><?php echo ($vo["id"]); ?></td>
                        <td class="add"><?php echo ($vo["banner_title"]); ?></td>
                        <td><img id="banner_img" height="50px" width="80px" src="<?php echo ($vo["banner_img"]); ?>" class="pimg" alt="头图图片" /></td>
                        <td><?php echo (translateBannerLocation($vo["banner_location_id"])); ?></td>
                        <td><?php echo (translateBannerType($vo["banner_type_id"])); ?></td>
                        <td><?php echo ($vo["banner_link"]); ?></td>
                        <td><?php echo ($vo["banner_sort"]); ?></td>
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/BannerInfo/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="BannerInfo_<?php echo ($k); ?>" data-href="<?php echo U('/BannerInfo/delete/id/'.$vo['id']);?>"
