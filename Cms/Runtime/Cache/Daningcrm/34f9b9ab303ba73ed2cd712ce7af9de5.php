...
<head>
    <meta charset="UTF-8">
    <title>
        头图信息 - 编辑 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <section class="panel">
        <div class="panel-body">
            ...
            <div class="form-group">   <label class="col-md-2 control-label">头图标题:<span class="required">*</span></label>   <div class="col-md-8">       <input  placeholder="头图标题:" type="text" class="form-control" name="<?php echo (CONTROLLER_NAME); ?>Banner_title" value="<?php echo ($vo["banner_title"]); ?>" required>      </div></div>
<!--            <div class="form-group">   <label class="col-md-2 control-label">头图图片:<span class="required">*</span></label>   <div class="col-md-8">       <input  placeholder="头图图片:" type="text" class="form-control" name="<?php echo (CONTROLLER_NAME); ?>Banner_img" value="<?php echo ($vo["banner_img"]); ?>" required>      </div></div>-->
            <input type="hidden" name = "BannerInfoBanner_img" id = "banner_img" value="<?php echo ($vo["banner_img"]); ?>" />

            <label class="col-md-2 control-label" for="textareaDefault">头图图片:</label>
            <img id="img-change" height="240px" width="320px" src="<?php echo ($vo["banner_img"]); ?>" id="img-change" class="banner_img" alt="头图图片" />
            <form id="form1" action="" method="post" enctype="multipart/form-data">
                <input type="file" name="upfile" id="upfile"  style="display:none;" onchange="filechange(event)" value="" />
            </form>

            <div class="form-group">   <label class="col-md-2 control-label">头图位置:<span class="required">*</span></label>   <div class="col-md-8">       <select data-plugin-selectTwo class="form-control" name="<?php echo (CONTROLLER_NAME); ?>Banner_location_id" required  >               <option value="">请选择</option>           <?php if(is_array($vo["BannerLocation"])): $i = 0; $__LIST__ = $vo["BannerLocation"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vos["key"]); ?>" <?php echo ($vos["readonly"]); ?> <?php if(($vos["key"]) == $vo["banner_location_id"]): ?>selected<?php endif; ?>><?php echo ($vos["label"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>       </select>   </div></div>
            <div class="form-group">   <label class="col-md-2 control-label">头图类型:<span class="required">*</span></label>   <div class="col-md-8">       <select data-plugin-selectTwo class="form-control" name="<?php echo (CONTROLLER_NAME); ?>Banner_type_id" required  >               <option value="">请选择</option>           <?php if(is_array($vo["BannerType"])): $i = 0; $__LIST__ = $vo["BannerType"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vos["key"]); ?>" <?php echo ($vos["readonly"]); ?> <?php if(($vos["key"]) == $vo["banner_type_id"]): ?>selected<?php endif; ?>><?php echo ($vos["label"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>       </select>   </div></div>
            <div class="form-group">   <label class="col-md-2 control-label">头图链接:<span class="required">*</span></label>   <div class="col-md-8">       <input  placeholder="头图链接:" type="text" class="form-control" name="<?php echo (CONTROLLER_NAME); ?>Banner_link" value="<?php echo ($vo["banner_link"]); ?>" required>      </div></div>
            <div class="form-group">   <label class="col-md-2 control-label">内部链接:</label>   <div class="col-md-8">       <select data-plugin-selectTwo class="form-control" name="<?php echo (CONTROLLER_NAME); ?>"   >               <option value="">请选择</option>           <?php if(is_array($vo["BannerLink"])): $i = 0; $__LIST__ = $vo["BannerLink"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vos["key"]); ?>" <?php echo ($vos["readonly"]); ?> <?php if(($vos["key"]) == $vo[""]): ?>selected<?php endif; ?>><?php echo ($vos["label"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>       </select>   </div></div>
            <div class="form-group">   <label class="col-md-2 control-label">头图排序:<span class="required">*</span></label>   <div class="col-md-8">       <input  placeholder="头图排序:" type="text" class="form-control" name="<?php echo (CONTROLLER_NAME); ?>Banner_sort" value="<?php echo ($vo["banner_sort"]); ?>" required>      </div></div>
        </div>
    ...
    <script>
        ...
        $("select[name='BannerInfo']").change(function () {
            innerLink = $("select[name='BannerInfo']").val();
            $("input[name='BannerInfoBanner_link']").val(innerLink);
        })
        ...
        var filechange=function(event){
            ...
            //    alert(imgURL);
                ...
                $.ajax({
                    type: "POST", // 数据提交类型
                    url: "https://daning-crm.cms.incker.com/api/upBanner",
                    data: formData, //发送数据
                    success:function (res) {
                        console.log(res);
                        if(res.code == 200){
                            $("input[name='BannerInfoBanner_img']").val(res.url);
                         //   alert(res.url);
                        }
