...
<head>
    <meta charset="UTF-8">
    <title>
        活动场次信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
    ...
    <section class="panel">
        <div class="panel-body">
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="ActivitySessionInfo_<?php echo ($k); ?>">
                        <td><?php echo ($vo["id"]); ?></td>
                        <td class="add"><?php echo ($vo["activity_name"]); ?></td>
                        <td ><?php echo ($vo["session_name"]); ?></td>
                        <td ><?php echo ($vo["session_date"]); ?></td>
                        <td ><?php echo ($vo["session_start_time"]); ?></td>
                        <td><?php echo ($vo["session_limit"]); ?></td>
                        <td><?php echo ($vo["confirm_user_count"]); ?></td>
                        <td>
                            <a href="javascript:void(0)" id = "signUpInfo_<?php echo ($vo['id']); ?>"  data-id="<?php echo ($vo['id']); ?>" class="signUpInfo btn btn-xs ml-sm mr-sm "> 报名信息</a>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/ActivityInfo/edit2/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="ActivitySessionInfo_<?php echo ($k); ?>" data-href="<?php echo U('/ActivityInfo/delete2/id/'.$vo['id']);?>"
    ...
    <script>
        function showpop() {
            ...
            $('.signUpInfo').click(function () {
                ...
                pop.fadeIn(300)
                $.ajax({
                    url: "https://daning-crm.cms.incker.com/api/getSessionSignUp",
                    type: 'post',
                    data: {
                        session_id:session_id
                    },
                    success: function (res) {
                        var list = res.signUpList;
                        data = list;
                        console.log(res)
                        if (res.code==200){
                            for (var i=0;i<list.length;i++) {

                                if (list[i]['sign_up_status'] == 4){
                                    btn = '<a href="javascript:;" data-ait="1">审核通过</a><a href="javascript:;" data-ait="2">审核拒绝</a><a href="javascript:;" data-ait="3">取消报名</a>'
                                }else {
                                    btn = '<a href="javascript:;" data-ait="3">取消报名</a>'
                                }
                                table.append('<tr  id="info_'+i+'"><th>'+
                                    ...
                                    +'</th><th>'+
                                    list[i]['session_name']
                                    +'</th><th class="look" data-trid="'+i+'" data-sign_up_id="'+list[i]["id"]+'">'+btn+'</th>'
                                )
                            }
                        }
            ...
            pop.on('click','.look a',function () {
                var that = $(this);
                sign_up_id =$(this).parents('.look').data("sign_up_id");
                ...
                switch (ait) {
                    case '1':
                        if (confirm("是否审核通过")) {
                            $.ajax({
                                url: "https://daning-crm.cms.incker.com/api/acceptUserSignUp",
                                type: 'post',
                                data: {
                    ...
                    case '2':
                        if (confirm("是否审核拒绝")) {
                            $.ajax({
                                url: "https://daning-crm.cms.incker.com/api/rejectUserSignUp",
                                type: 'post',
                                data: {
                                    sign_up_id: sign_up_id
                                },
                    ...
                    case '3':
                        if (confirm("是否取消该用户的预定")) {
                            $.ajax({
                                url: "https://daning-crm.cms.incker.com/api/cancelUserSignUp",
                                type: 'post',
                                data: {
                                    sign_up_id: sign_up_id
                                },
