...
<head>
    <meta charset="UTF-8">
    <title>
        问题咨询 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <section class="panel">
        <div class="panel-body">
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="ActivitySessionInfo_<?php echo ($k); ?>">
                        ...
                        <td class="add"><?php echo ($vo["user_question"]); ?></td>
                        <td>
                            <a href="javascript:void(0)" id = "userQuestionInfo_<?php echo ($vo['id']); ?>"  data-id="<?php echo ($vo['id']); ?>" class="userQuestionInfo btn btn-xs ml-sm mr-sm "> 反馈</a>
                        </td>
        ...
        <div class="list_pop">
            <div class="list11">
                <label class="questionLabel" >问题反馈:</label>
                <textarea  id="user_question" rows="5"  class="userQuestion" ></textarea>
                <br>
            <a href="javascript:void(0)" id = "look"   class="userQuestionInfo2 btn btn-xs ml-sm mr-sm "> 提交</a>
            <a href="javascript:void(0)" id = "cancel"   class="userQuestionInfo2 btn btn-xs ml-sm mr-sm "> 取消</a>
            </div>
<!--            <div class="close"></div>-->
        </div>
...
<!-- Specific -->
    ...
    <script>
        ...
        function showpop() {
            ...
            $('.userQuestionInfo').click(function () {
                question_id =$(this).attr("data-id");
                console.log("111："+question_id);
                pop.fadeIn(300)
            });

            pop.on('click','#look',function () {

                question_answer = $('#user_question').val();
                console.log("question："+question_id);
                if(confirm("是否提交用户问题反馈？"))

                {
                    $.ajax({
                        url: "https://daning-crm.cms.incker.com/api/answerUserQuestion",
                        type: 'post',
                        data: {
