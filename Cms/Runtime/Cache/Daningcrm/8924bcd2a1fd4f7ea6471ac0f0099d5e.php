...
<head>
    <meta charset="UTF-8">
    <title>
        问卷内容编辑 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
    ...
    <section class="panel">
        <div class="panel-body">
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="SurveyInfo_<?php echo ($k); ?>">
                        <td><?php echo ($vo["id"]); ?></td>
                        <td><?php echo ($vo["survey_name"]); ?></td>
                        <td><?php echo ($vo["survey_title"]); ?></td>
                        <td><?php echo (translateSurveyType($vo["survey_type"])); ?></td>
                        <td><?php echo ($vo["survey_summary"]); ?></td>
                        <td><?php echo ($vo["survey_sort"]); ?></td>
                        <td>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="SurveyInfo_<?php echo ($k); ?>" data-href="<?php echo U('/SurveyDetails/delete/id/'.$vo['id']);?>"
                                   data-toggle="confirmation" class="btn btn-xs btn-primary"><i
                                        class="fa fa-trash-o"></i> 删除</a>
                                <a href="<?php echo U('/SurveyDetails/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                        </td>
