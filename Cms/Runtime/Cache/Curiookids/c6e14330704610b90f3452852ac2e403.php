...
<head>
    <meta charset="UTF-8">
    <title>
        渠道信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <a href="https://curiookids.incker.com/ActivityInfo/insert2.html" class="ktx-form-btn" id="select-btn">
        新增
    </a>
    <section class="panel">
        <div class="panel-body">
            <table class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                    <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="ChannelInfo_<?php echo ($k); ?>">
                            <td><?php echo ($vo["id"]); ?></td>
                            <td class="add"><?php echo ($vo["channel_name"]); ?></td>
                            <td>
                                <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="ChannelInfo_<?php echo ($k); ?>"
                                        data-href="<?php echo U('/ActivityInfo/delete2/id/'.$vo['id']);?>"
