...
<head>
    <meta charset="UTF-8">
    <title>
        账户管理 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <a href="https://curiookids.incker.com/UserAdmin/insert.html" class="ktx-form-btn" id="select-btn">
        新增
    </a>
    <section class="panel">
            ...
            <table class="table table-bordered table-condensed mb-none">
                <thead>
                    <tr>
                        ...
                        <th>员工姓名</th>
                        <th width="100px">所在地区</th>
                        <th width="120px">所属门店</th>
                        <th width="75px">权限</th>
                        <th width="75px">是否删除</th>
                        <th>操作</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="UserAdmin_<?php echo ($k); ?>">
                            <td><?php echo ($vo["id"]); ?></td>
                            <td><?php echo ($vo["username"]); ?></td>
                            <td class="add"><?php echo ($vo["nickname"]); ?></td>
                            <td class="add"><?php echo ($vo["area_name"]); ?></td>
                            <td class="add"><?php echo ($vo["store_name"]); ?></td>
                            <td class="add"><?php echo (translateGroup($vo["group_id"])); ?></td>
                            <td class="add"><?php echo (translateDelFlg($vo["del_flg"])); ?></td>
                            <td>
                                <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/UserAdmin/edit2/id/'.$vo['id']);?>"
                                        class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 重置密码</a><?php endif; ?>
                                <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/UserAdmin/edit/id/'.$vo['id']);?>"
                                        class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                                <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="UserAdmin_<?php echo ($k); ?>" data-href="<?php echo U('/UserAdmin/delete/id/'.$vo['id']);?>"
