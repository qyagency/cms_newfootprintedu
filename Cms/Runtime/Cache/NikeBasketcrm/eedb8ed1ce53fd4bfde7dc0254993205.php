...
<head>
    <meta charset="UTF-8">
    <title>
        比赛信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <section class="panel">
        ...
        <div class="panel-body">
            <span class="btn btn-xs btn-primary ml-sm mr-sm updaterange">更新排行</span>
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="BasketQuestionDetails_<?php echo ($k); ?>">
                        <td><?php echo ($vo["id"]); ?></td>
                        <td><?php echo ($vo["question_id"]); ?></td>
                        <td><?php echo ($vo["match_title"]); ?></td>
                        <td><?php echo ($vo["match_date"]); ?></td>
                        <td><?php echo ($vo["match_time"]); ?></td>
                        <td><?php echo ($vo["match_team_a"]); ?></td>
                        <td><?php echo ($vo["match_team_b"]); ?></td>
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/BasketQuestionDetails/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="BasketQuestionDetails_<?php echo ($k); ?>" data-href="<?php echo U('/BasketQuestionDetails/delete/id/'.$vo['id']);?>"
    ...
    <script>
        ...
        $('.updaterange').click(function () {
         ...
         var boo = confirm('确认更新用户答题积分?')
          //confirm 会返回你选择的选项,然后可以依据选择执行逻辑
          if(boo){
          $.ajax({
          url: "http://nikebasket-crm.cms.incker.com/api/updateUserRange",
          type: 'post',
