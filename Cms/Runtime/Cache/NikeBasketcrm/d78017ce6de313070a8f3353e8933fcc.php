...
<head>
    <meta charset="UTF-8">
    <title>
        竞猜信息 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <section class="panel">
        <div class="panel-body">
            ...
            <table id="datatable" class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="BasketQuestionInfo_<?php echo ($k); ?>">
                        <td><?php echo ($vo["id"]); ?></td>
                        <td><?php echo ($vo["question_date"]); ?></td>
                        <td><?php echo ($vo["question_title"]); ?></td>
                        <td><?php echo ($vo["next_title"]); ?></td>
                        <td><?php echo ($vo["next_date"]); ?></td>
                        <td><?php echo ($vo["next_match1"]); ?></td>
                        <td><?php echo ($vo["next_match2"]); ?></td>
                        <td><?php echo ($vo["next_guess_time"]); ?></td>
                        <td width="400">
                            <span data-id="<?php echo ($vo["id"]); ?>" class="btn btn-xs btn-primary ml-sm mr-sm updaterange">上线</span>
                            <span data-id="<?php echo ($vo["id"]); ?>" class="btn btn-xs btn-primary ml-sm mr-sm endguess">停止竞猜</span>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/BasketQuestionInfo/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="BasketQuestionInfo_<?php echo ($k); ?>" data-href="<?php echo U('/BasketQuestionInfo/delete/id/'.$vo['id']);?>"
    ...
    <script>
        ...
        $('.updaterange').click(function () {
            ...
            var id = $(this).data("id");
            var boo = confirm('确认是否上线当前日期比赛竞猜?')

            //confirm 会返回你选择的选项,然后可以依据选择执行逻辑
            if(boo){
                $.ajax({
                    url: "http://nikebasket-crm.cms.incker.com/api/updateQuestionStatus",
                    type: 'post',
                    data:{"id":id},
        ...
        //发送邮件
        $('.endguess').click(function () {

            var id = $(this).data("id");
            var boo = confirm('确认是否停止当前日期比赛竞猜?')

            //confirm 会返回你选择的选项,然后可以依据选择执行逻辑
            if(boo){
                $.ajax({
                    url: "http://nikebasket-crm.cms.incker.com/api/endQuestion",
                    type: 'post',
                    data:{"id":id},
                    success: function (res) {
                        if(res==200){
