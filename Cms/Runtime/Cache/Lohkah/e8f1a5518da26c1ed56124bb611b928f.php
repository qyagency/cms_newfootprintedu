...
<head>
    <meta charset="UTF-8">
    <title>
        权限管理 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <section class="panel">
        <?php if(in_array(($cRefresh), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><div class="alert alert-warning">
        ...
        <div class="panel-body">
            <table id="datatable" class="table table-bordered table-striped table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="group_<?php echo ($i); ?>">
                        <td><?php echo ($vo["id"]); ?></td>
                        <td><?php echo ($vo["name"]); ?>[<?php echo ($vo["cnt"]); ?>]</td>
                        <td><?php echo (date("m/d H:i",$vo["create_time"])); ?></td>
                        <td><?php echo (date("m/d H:i",$vo["update_time"])); ?></td>
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/Group/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a <?php if(($vo["del_flg"]) == "-1"): ?>disabled<?php endif; ?> data-target="group_<?php echo ($i); ?>" data-href="<?php echo U('/Group/delete/id/'.$vo['id']);?>" data-toggle="confirmation" class="btn btn-xs btn-primary"><i class="fa fa-trash-o"></i> 删除</a><?php endif; ?>
                        </td>
