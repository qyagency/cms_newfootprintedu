...
<head>
    <meta charset="UTF-8">
    <title>
        用户管理 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <section class="panel">
        <div class="panel-body media-gallery">
            <div class="row mg-files pb-none">
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="col-md-4" id="user_<?php echo ($i); ?>">
                        <div class="thumbnail">
                            <div class="thumb-preview">
                                <a class="thumb-image" href="assets/images/projects/project-1.jpg">
                                    <img src="http://bojia-hs.oss-cn-shanghai.aliyuncs.com/./User/empty.jpg" class="img-responsive">
                                </a>
                                <div class="mg-thumb-options">
                                    <div class="mg-toolbar">
                                        <div class="mg-group pull-left">
                                            <?php echo (date("m/d H:i",$vo["last_login_time"])); ?>
                                        </div>
                                        <div class="mg-group pull-right">
                                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-toggle="tooltip" data-original-title="编辑" href="<?php echo U('/user/edit/id/'.$vo['id']);?>"><i class="fa fa-pencil"></i></a><?php endif; ?>
                                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): if(($vo["del_flg"]) == "-1"): ?><a data-toggle="tooltip" data-original-title="锁定"><i class="fa fa-lock"></i></a>
                                                    <?php else: ?>
                                                    <a data-toggle="asyn" data-original-title="删除" data-target="user_<?php echo ($i); ?>" data-href="<?php echo U('/user/delete/id/'.$vo['id']);?>"><i class="fa fa-trash-o"></i></a><?php endif; endif; ?>
                                        </div>
                                    </div>
                                </div>

                            </div>
                            <h5 class="mg-title text-weight-semibold"><?php echo ($vo["username"]); ?><small>（<?php echo ($vo["nickname"]); ?>）</small></h5>
                            <div class="mg-description">
                                <small class="pull-left text-muted"><?php echo ($groupList[$vo['group_id']]); ?></small>
                                <small class="pull-right text-muted"><?php echo (translateSts($vo["status"])); ?></small>
                            </div>
                        </div>
