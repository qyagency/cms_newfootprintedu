...
<head>
    <meta charset="UTF-8">
    <title>
        Gallery管理 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <!--<div class="alert alert-danger">-->
        ...
        <!--<strong>已经上架的项目无法进行编辑、删除，已经存在订单的项目无法删除</strong>-->
    <!--</div>-->
    <section class="panel">
        <div class="panel-body">
            <table id="datatable2" class="table table-bordered table-condensed mb-none">
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="gallery_<?php echo ($k); ?>">
                        ...
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/Gallery/edit/id/'.$vo['id']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="gallery_<?php echo ($k); ?>" data-href="<?php echo U('/Gallery/delete/id/'.$vo['id']);?>"
