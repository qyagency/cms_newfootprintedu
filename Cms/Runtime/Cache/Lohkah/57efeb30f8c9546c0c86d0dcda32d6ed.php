...
<head>
    <meta charset="UTF-8">
    <title>
        RoomsCover管理 - 列表 | HAPPY-SHARE Admin - Responsive HTML5 CMS
    </title>
...
<section class="body">
    ...
    <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/RoomsCover/insert/roomsId/'.$_GET['roomsId']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 新增</a><?php endif; ?>
    <section class="panel">
        <div class="panel-body">
            <table id="datatable2" class="table table-bordered table-condensed mb-none">
                <thead>
                <tr>
                    <th>#</th>
                    <th>类型</th>
                ...
                <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr id="rooms_cover_<?php echo ($k); ?>">
                        <td><?php echo ($vo["id"]); ?></td>
                        <td><?php echo ($vo["type_id"]); ?></td>
                        <td><img src="/Public/Uploads/Lohkah/<?php echo ($vo["pc_cover"]); ?>" height="30" /></td>
                        <td><img src="/Public/Uploads/Lohkah/<?php echo ($vo["mobile_cover"]); ?>" height="30" /></td>
                        <td><?php echo ($vo["sort"]); ?></td>
                        <td>
                            <?php if(in_array(($cEdit), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a href="<?php echo U('/RoomsCover/edit/id/'.$vo['id'].'/roomsId/'.$_GET['roomsId']);?>" class="btn btn-xs btn-primary ml-sm mr-sm"><i class="fa fa-pencil"></i> 编辑</a><?php endif; ?>
                            <?php if(in_array(($cDelete), is_array($_SESSION['routes'])?$_SESSION['routes']:explode(',',$_SESSION['routes']))): ?><a data-target="rooms_cover_<?php echo ($k); ?>" data-href="<?php echo U('/RoomsCover/delete/id/'.$vo['id']);?>"
