...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
    ...
    function drawScale($Data, $DataDescription, $ScaleMode, $R, $G, $B, $DrawTicks = TRUE, $Angle = 0, $Decimals = 1, $WithMargin = FALSE, $SkipLabels = 1, $RightScale = FALSE) {
        ...
        if (count($Data) > 1) {
            if ($WithMargin == FALSE)
                $this->DivisionWidth = ($this->GArea_X2 - $this->GArea_X1) / (count($Data) - 1); else {
        ...
        for ($i = 1; $i <= $Divisions + 1; $i++) {
            if ($RightScale)
                $this->drawLine($this->GArea_X2, $YPos, $this->GArea_X2 + 5, $YPos, $R, $G, $B); else
        ...
        if (isset($DataDescription["Axis"]["Y"])) {
            ...
            if ($RightScale)
                imagettftext($this->Picture, $this->FontSize, 90, $XMin + $this->FontSize, $TextTop, $C_TextColor, $this->FontName, $DataDescription["Axis"]["Y"]); else
        ...
        foreach ($Data as $Key => $Values) {
            if ($ID % $SkipLabels == 0) {
                ...
                if ($Angle == 0) {
                    ...
                    if ($Angle <= 90)
                        imagettftext($this->Picture, $this->FontSize, $Angle, floor($XPos) - $TextWidth + 5, $YPos, $C_TextColor, $this->FontName, $Value); else
    ...
    function drawXYScale($Data, $DataDescription, $YSerieName, $XSerieName, $R, $G, $B, $WithMargin = 0, $Angle = 0, $Decimals = 1) {
        ...
        if ($this->VMin == NULL && $this->VMax == NULL) {
            ...
            if ($this->isRealInt(($this->VMax - $this->VMin) / ($Divisions - 1)))
                $Divisions--; elseif ($this->isRealInt(($this->VMax - $this->VMin) / ($Divisions + 1)))
                $Divisions++;
        } else
            $Divisions = $this->Divisions;
        ...
        if ($this->VXMin == NULL && $this->VXMax == NULL) {
            ...
            if ($this->isRealInt(($this->VXMax - $this->VXMin) / ($XDivisions - 1)))
                $XDivisions--; elseif ($this->isRealInt(($this->VXMax - $this->VXMin) / ($XDivisions + 1)))
                $XDivisions++;
        } else
    ...
    function drawTreshold($Value, $R, $G, $B, $ShowLabel = FALSE, $ShowOnRight = FALSE, $TickWidth = 4, $FreeText = NULL) {
        ...
        if ($TickWidth == 0)
            $this->drawLine($this->GArea_X1, $Y, $this->GArea_X2, $Y, $R, $G, $B); else
        ...
        if ($ShowLabel) {
            ...
            if ($ShowOnRight)
                imagettftext($this->Picture, $this->FontSize, 0, $this->GArea_X2 + 2, $Y + ($this->FontSize / 2), $C_TextColor, $this->FontName, $Label); else
            ...
            foreach ($Data as $Key => $Values) {
                ...
                if (is_numeric($Value)) {
                    if (!isset($DataDescription["Symbol"][$ColName])) {
                        ...
                        if ($Shadow) {
                            if ($R3 != -1 && $G3 != -1 && $B3 != -1)
                                $this->drawFilledCircle($XPos + 2, $YPos + 2, $BigRadius, $R3, $G3, $B3); else {
                        ...
                        if ($SmallRadius != 0) {
                            if ($R2 != -1 && $G2 != -1 && $B2 != -1)
                                $this->drawFilledCircle($XPos + 1, $YPos + 1, $SmallRadius, $R2, $G2, $B2); else {
    ...
    function drawXYPlotGraph($Data, $DataDescription, $YSerieName, $XSerieName, $PaletteID = 0, $BigRadius = 5, $SmallRadius = 2, $R2 = -1, $G2 = -1, $B2 = -1, $Shadow = TRUE) {
        ...
        foreach ($Data as $Key => $Values) {
            if (isset($Data[$Key][$YSerieName]) && isset($Data[$Key][$XSerieName])) {
                ...
                if ($Shadow) {
                    if ($R3 != -1 && $G3 != -1 && $B3 != -1)
                        $this->drawFilledCircle($X + 2, $Y + 2, $BigRadius, $R3, $G3, $B3); else {
                ...
                if ($R2 != -1 && $G2 != -1 && $B2 != -1)
                    $this->drawFilledCircle($X + 1, $Y + 1, $SmallRadius, $R2, $G2, $B2); else {
    ...
    function drawStackedBarGraph($Data, $DataDescription, $Alpha = 50, $Contiguous = FALSE) {
        ...
        $Series = count($DataDescription["Values"]);
        if ($Contiguous)
            $SeriesWidth = $this->DivisionWidth; else
        ...
        foreach ($iValues as $Key => $Value) {
            ...
            if (!($DrawLabels == PIE_NOLABEL)) {
                ...
                if ($DrawLabels == PIE_PERCENTAGE)
                    $Caption = (round($Value * pow(10, $Decimals) * $SplicePercent) / pow(10, $Decimals)) . "%"; elseif ($DrawLabels == PIE_LABELS)
                    $Caption = $iLabels[$Key];
                ...
                if ($TAngle > 0 && $TAngle < 180)
                    $TY = sin(($TAngle) * 3.1418 / 180) * ($Radius + 10) + $YPos + 4; else
    ...
    function drawFlatPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius = 100, $DrawLabels = PIE_NOLABEL, $SpliceDistance = 0, $Decimals = 0, $AllBlack = FALSE) {
        ...
        foreach ($iValues as $Key => $Value) {
            ...
            if (!($DrawLabels == PIE_NOLABEL)) {
                ...
                if ($TAngle > 0 && $TAngle < 180)
                    $TY = sin(($TAngle) * 3.1418 / 180) * ($Radius + 10 + $SpliceDistance) + $YPos + 4; else
            ...
            /* Process pie slices */
            if (!$AllBlack)
                $LineColor = $this->AllocateColor($this->Picture, $Rc, $Gc, $Bc); else
        ...
        foreach ($PolyPlots as $Key => $Value) {
            if (!$AllBlack)
                $C_GraphLo = $this->AllocateColor($this->Picture, $this->Palette[$Key]["R"], $this->Palette[$Key]["G"], $this->Palette[$Key]["B"]); else
    ...
    function drawPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius = 100, $DrawLabels = PIE_NOLABEL, $EnhanceColors = TRUE, $Skew = 60, $SpliceHeight = 20, $SpliceDistance = 0, $Decimals = 0) {
        ...
        foreach ($iValues as $Key => $Value) {
            ...
            if (!($DrawLabels == PIE_NOLABEL)) {
                $TAngle = $Angle + ($Value * $SpliceRatio / 2);
                if ($DrawLabels == PIE_PERCENTAGE)
                    $Caption = (round($rValues[$Key] * pow(10, $Decimals) * $rSplicePercent) / pow(10, $Decimals)) . "%"; elseif ($DrawLabels == PIE_LABELS)
                    $Caption = $iLabels[$Key];
                elseif ($DrawLabels == PIE_PERCENTAGE_LABEL)
                    $Caption = $iLabels[$Key] . "\r\n" . (round($Value * pow(10, $Decimals) * $SplicePercent) / pow(10, $Decimals)) . "%";
                ...
                if ($TAngle > 0 && $TAngle < 180)
                    $TY = sin(($TAngle) * 3.1418 / 180) * ($SkewHeight + 10) + $YPos + $SpliceHeight + 4; else
    ...
    function drawLine($X1, $Y1, $X2, $Y2, $R, $G, $B, $GraphFunction = FALSE) {
        ...
        for ($i = 0; $i <= $Distance; $i++) {
            ...
            if (($X >= $this->GArea_X1 && $X <= $this->GArea_X2 && $Y >= $this->GArea_Y1 && $Y <= $this->GArea_Y2) || !$GraphFunction) {
                if ($this->LineWidth == 1)
                    $this->drawAntialiasPixel($X, $Y, $R, $G, $B); else {
